...
class Game
{
	...
	void dispProjectileInfo() {
		std::list<Projectile> curProjectiles = getProjectiles();
		std::cout << "Projectile Information at time: " << getGameTime() << std::endl;
		for(auto it = curProjectiles.begin(); it!= curProjectiles.end(); ++it)
		{
			std::cout << "Projectile: vx:" << it->vx << " vy:" << it->vy << std::endl;  
		}
	}
